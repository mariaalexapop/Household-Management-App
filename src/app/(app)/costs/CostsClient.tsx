  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        ...
        <Card className="bg-kinship-surface-container-lowest p-4">
          <div className="absolute left-0 top-0 h-1 w-full rounded-t-2xl bg-[#16a34a]" aria-hidden="true" />
          <p className="font-body text-xs text-kinship-on-surface-variant">Medical</p>
          <p className="font-display text-lg font-semibold text-kinship-on-surface">
            {formatOrDash(medicalTotal)}
          </p>
        </Card>
        <Card className="bg-kinship-surface-container-lowest p-4">
          <div className="absolute left-0 top-0 h-1 w-full rounded-t-2xl bg-[#2563eb]" aria-hidden="true" />
          <p className="font-body text-xs text-kinship-on-surface-variant">Home</p>
          <p className="font-display text-lg font-semibold text-kinship-on-surface">
            {formatOrDash(homeTotal)}
          </p>
        </Card>
        <Card className="bg-kinship-surface-container-lowest p-4">
          ...
          <table className="w-full min-w-[540px]">
            <thead className="border-b border-kinship-surface-container bg-kinship-surface-container/30">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-display text-sm font-semibold text-kinship-on-surface">Car</th>
                <th className="px-4 py-3 text-right font-display text-sm font-semibold text-kinship-on-surface">Medical</th>
                <th className="px-4 py-3 text-right font-display text-sm font-semibold text-kinship-on-surface">Home</th>
            ...
            <tbody>
              {monthlyRows.map((r) => {
                const rowTotal = r.carCents + r.medicalCents + r.homeCents + r.electronicsCents
                return (
                  <tr key={r.monthKey} className="border-b border-kinship-surface-container last:border-b-0">
                    ...
                    <td className="px-4 py-2.5 text-right font-body text-sm text-kinship-on-surface tabular-nums">
                      {formatOrDash(r.medicalCents)}
                    </td>
                    <td className="px-4 py-2.5 text-right font-body text-sm text-kinship-on-surface tabular-nums">
                      {formatOrDash(r.homeCents)}
                    </td>
                    <td className="px-4 py-2.5 text-right font-body text-sm text-kinship-on-surface tabular-nums">
