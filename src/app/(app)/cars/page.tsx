...
export default async function CarsPage() {
  ...
  const serializedCars = carRows.map((c) => ({
    ...
    motCostCents: c.motCostCents ?? null,
    motPaymentDate: c.motPaymentDate ? c.motPaymentDate.toISOString() : null,
    taxDueDate: c.taxDueDate ? c.taxDueDate.toISOString() : null,
    taxCostCents: c.taxCostCents ?? null,
    taxPaymentDate: c.taxPaymentDate ? c.taxPaymentDate.toISOString() : null,
  ...
  const serializedServiceRecords = recordRows.map((r) => ({
    ...
    serviceType: r.serviceType as 'full_service' | 'mot' | 'road_tax' | 'repair' | 'tyre' | 'other',
